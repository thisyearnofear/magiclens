...
import asyncio
import json
from fastapi import Query, FastAPI, WebSocket, WebSocketDisconnect, Request
...
@app.websocket("/")
async def root_websocket_endpoint(websocket: WebSocket, key: str = Query(None)):
    loggingSecretKey = os.environ.get("LOGGING_SECRET_KEY")
    if loggingSecretKey is None or key is None or key != loggingSecretKey:
